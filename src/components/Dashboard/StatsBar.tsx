...
// Internal interface for stat items
interface StatItem {
    icon: LucideIcon
    label: string
    value: number
    goal?: number
    stats_goal?: number // keeping original name semantics if needed, but likely `goal`
    progress?: number
    color: string
    bgColor: string
    subtext?: string
...
export const StatsBar = memo(function StatsBar({ stats, streak, onOpenSettings, goalsEnabled = true }: StatsBarProps) {
    ...
    const statItems = useMemo(() => {
        ...
        items.push({
            ...
            subtext: stats.lateCount && stats.lateCount > 0 ? `${stats.lateCount} overdue` : 'on track',
            color: stats.lateCount && stats.lateCount > 0 ? 'text-accent-danger' : 'text-accent-warning',
            bgColor: stats.lateCount && stats.lateCount > 0 ? 'bg-accent-danger/20' : 'bg-accent-warning/20'
        })
        items.push({
            icon: CheckCircle,
            label: 'Published',
    ...
    return (
        ...
        >
            ...
            {goalsEnabled ? (
                <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${insights.isAhead
                    ...
                    <div className="ml-auto flex items-center gap-3">
                        <div className="flex items-center gap-2">
                            <div className="w-24 h-2 bg-bg-tertiary rounded-full overflow-hidden">
                                <div
                                    className={`h-full rounded-full transition-all duration-500 ${insights.isAhead ? 'bg-accent-success' : insights.isBehind ? 'bg-accent-danger' : 'bg-accent-primary'
                                        }`}
