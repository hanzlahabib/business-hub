...
import { CALENDAR_ITEM_TYPES, CalendarItem } from '../../hooks/useCalendarItems'
...
interface CalendarItemCardProps {
    ...
    onDragEnd?: (item: CalendarItem) => void
}
                ...
                {item.type === 'task' && item.sourceData?.subtasks?.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-border/30">
                        <p className="text-[10px] text-text-muted">
                            {item.sourceData.subtasks.filter((s: any) => s.done).length}/{item.sourceData.subtasks.length} subtasks
                        </p>
                    </div>
                )}
