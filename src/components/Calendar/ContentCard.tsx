...
import { Video, Smartphone, MoreVertical, Trash2, Edit, GripVertical, MessageSquare, Youtube, FileText, FileCode, Link2, Presentation, Monitor, Pencil, User, LucideIcon } from 'lucide-react'
import { Badge } from '../ui/badge'
import { useState, useCallback, useMemo, memo } from 'react'

const URL_ICONS: Record<string, { icon: LucideIcon; color: string }> = {
...
const VIDEO_VARIANT_CONFIG: Record<string, { icon: LucideIcon; color: string; label: string }> = {
...
interface VideoVariantBadgeProps {
    variant?: string
    size?: 'default' | 'icon' | 'small'
}

function VideoVariantBadge({ variant, size = 'default' }: VideoVariantBadgeProps) {
    ...
    if (size === 'icon') {
        return <VariantIcon size={10} style={{ color: config.color }} />
    }

    if (size === 'small') {
        return <VariantIcon size={12} style={{ color: config.color }} />
    }
...
const statusColors: Record<string, "default" | "primary" | "secondary" | "success" | "warning" | "danger" | "outline"> = {
    idea: 'default',
    script: 'warning',
    recording: 'primary',
    editing: 'secondary',
...
import { Content, Comment, Url } from '../../hooks/useSchedule'

export type { Content, Comment, Url }

interface ContentCardProps {
    content: Content
    viewMode?: 'compact' | 'default' | 'detailed' | string
    onEdit?: (content: Content) => void
    onDelete?: (id: string) => void
    onClick?: (content: Content) => void
}

export const ContentCard = memo(function ContentCard({ content, viewMode = 'default', onEdit, onDelete, onClick }: ContentCardProps) {
            ...
            >
                ...
                <div className="flex items-center gap-1">
                    ...
                    {content.presentationReady && (
                        <Presentation size={10} className="text-accent-success" />
                    )}
                </div>
                <span className={`w-2 h-2 rounded-full ${content.status === 'published' ? 'bg-accent-success' :
