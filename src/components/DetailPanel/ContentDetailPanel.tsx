...
import { X, Edit3, Calendar, Clock, Tag, FileText, ExternalLink, Youtube, FileCode, Link2, Trash2, Plus, Presentation, Folder, ListChecks, LucideIcon } from 'lucide-react'
import { format, parseISO } from 'date-fns'
import { motion, AnimatePresence } from 'framer-motion'
import { CommentSection } from '../Comments'
import { Content } from '@/components/Calendar/ContentCard'

const URL_TYPE_CONFIG: Record<string, { icon: LucideIcon; color: string; bgColor: string; borderColor: string; label: string }> = {
    youtube: {
        icon: Youtube,
        color: 'text-red-500',
...
interface ContentDetailPanelProps {
    ...
    onAddUrl: (contentId: string, urlData: { url: string; label: string }) => Promise<void>
    onRemoveUrl: (contentId: string, urlId: string) => Promise<void>
...
export const ContentDetailPanel = memo(function ContentDetailPanel({
    ...
    const handleAddUrl = useCallback(async (e: React.FormEvent) => {
        e.preventDefault()
        if (!newUrl.trim() || !content) return
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-border">
                            <div className="flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded text-xs font-medium ${content.type === 'long' ? 'bg-accent-primary/20 text-accent-primary' : 'bg-accent-secondary/20 text-accent-secondary'
                                    }`}>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-6">
                            ...
                            <div className="flex items-center justify-between p-3 rounded-lg bg-bg-tertiary/50 border border-border">
                                ...
                                <button
                                    ...
                                    className={`relative w-11 h-6 rounded-full transition-colors ${content.presentationReady ? 'bg-accent-success' : 'bg-border'
                                        }`}
                                >
                                    <span
                                        className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${content.presentationReady ? 'translate-x-5' : 'translate-x-0'
                                            }`}
                                    />
                                </button>
                            </div>
                                        ...
                                        {content.slideDetails.bulletPoints?.length > 0 && (
                                            <div>
                                                ...
                                                <ul className="space-y-1 ml-1">
                                                    {content.slideDetails.bulletPoints.map((point: string, index: number) => (
                                        ...
                                        {content.slideDetails.slides?.length > 0 && (
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-1.5">
                                                    {content.slideDetails.slides.map((slide: string, index: number) => (
                                                        <span
                                                            key={index}
...
const UrlItem = memo(function UrlItem({ urlItem, onRemove }: { urlItem: any, onRemove: () => void }) {
