...
import { ChevronLeft, ChevronRight, Video, Smartphone, Calendar } from 'lucide-react'
import { format, parseISO, addDays, startOfWeek, addWeeks, isSameDay, isToday, isBefore } from 'date-fns'
...
export const TimelineView = memo(function TimelineView({ contents, onEdit, onSchedule, onOpenDetail }) {
          ...
          {dates.map(date => {
            ...
            return (
              <div
                ...
                className={`flex-shrink-0 w-[140px] rounded-xl border transition-colors ${
                  isToday(date)
              ...
              >
                ...
                <div className={`p-2 border-b text-center ${
                  isToday(date) ? 'border-accent-primary/30' : 'border-border/50'
                }`}>
                  <p className={`text-xs ${isToday(date) ? 'text-accent-primary font-medium' : 'text-text-muted'}`}>
                    {format(date, 'EEE')}
                  </p>
                  <p className={`text-lg font-semibold ${
                    isToday(date) ? 'text-accent-primary' : 'text-text-primary'
                  }`}>
                    {format(date, 'd')}
                  </p>
                </div>
...
const TimelineCard = memo(function TimelineCard({ content, onEdit, onOpenDetail }) {
  ...
  return (
    <motion.div
      ...
      className={`p-2 rounded-lg border cursor-grab active:cursor-grabbing transition-all hover:scale-[1.02] ${
        isLong
