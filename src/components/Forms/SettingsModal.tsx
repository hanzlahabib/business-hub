...
export const SettingsModal = memo(function SettingsModal({
    ...
    const handleLongChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        setLongGoal(parseInt(e.target.value) || 0)
    }, [])

    const handleShortsChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        setShortsGoal(parseInt(e.target.value) || 0)
    }, [])
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <button
                                ...
                                className={`relative w-11 h-6 rounded-full transition-colors ${goalsEnabled ? 'bg-accent-primary' : 'bg-bg-tertiary border border-border'
                                    }`}
                            >
                                <span
                                    className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${goalsEnabled ? 'left-6' : 'left-1'
                                        }`}
                                />
                            </button>
                        </div>
