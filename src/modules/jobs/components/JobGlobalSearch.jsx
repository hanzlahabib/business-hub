        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`px-1.5 py-0.5 rounded text-xs ${statusConfig.color || 'bg-gray-500/20 text-gray-400'}`}>
              {statusConfig.emoji} {statusConfig.label || job.status}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-text-muted">
            {job.location && (
              <span className="flex items-center gap-1">
                ...
                <HighlightText text={job.location} query={query} />
              </span>
            )}
            {job.salary && (
              <span className="flex items-center gap-1">
          ...
          {job.skills?.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {job.skills.slice(0, 4).map(skill => (
                <span key={skill} className="px-1.5 py-0.5 bg-blue-500/10 rounded text-xs text-blue-400">
                  <HighlightText text={skill} query={query} />
                </span>
              ))}
              {job.skills.length > 4 && (
                <span className="px-1.5 py-0.5 text-xs text-text-muted">+{job.skills.length - 4}</span>
              )}
            </div>
          )}
        </div>
...
export function JobGlobalSearch({
  jobs,
  isOpen,
  onClose,
  onSelectJob
  ...
  const performSearch = useCallback((searchQuery) => {
    ...
    for (const job of jobs) {
      ...
      // Search in company
      else if (job.company?.toLowerCase().includes(searchLower)) {
        matchField = 'company'
      }
      // Search in location
      else if (job.location?.toLowerCase().includes(searchLower)) {
        matchField = 'location'
      }
      // Search in skills
      else if (job.skills?.some(s => s.toLowerCase().includes(searchLower))) {
        matchField = 'skills'
      }
      // Search in description
      else if (job.description?.toLowerCase().includes(searchLower)) {
        matchField = 'description'
      }
      // Search in notes
      else if (job.notes?.toLowerCase().includes(searchLower)) {
        matchField = 'notes'
      }

      if (matchField) {
        searchResults.push({ job, matchField })
      }
    }
          ...
          {query.length < 2 ? (
            <div className="p-8 text-center text-text-muted">
              ...
              <p className="text-xs mt-2 text-text-muted/70">
                Searches role, company, location, skills, description, notes
              </p>
            </div>
          ) : results.length === 0 ? (
            <div className="p-8 text-center text-text-muted">
...
export default JobGlobalSearch
