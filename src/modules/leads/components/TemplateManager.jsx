...
function TemplateCard({ template, onEdit, onDelete }) {
  ...
  return (
    <div className="p-4 bg-white/5 border border-white/10 rounded-xl hover:border-white/20 transition-colors">
      ...
      {template.variables && template.variables.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-3">
          {template.variables.map(v => (
            <span key={v} className="px-1.5 py-0.5 bg-white/10 rounded text-[10px] text-white/50">
              {`{{${v}}}`}
            </span>
          ))}
        </div>
...
function TemplateEditor({ template, onSave, onCancel }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="text-sm text-white/60 mb-2 block">Subject Line *</label>
        <input
          type="text"
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-sm text-white/60">Email Body *</label>
          <button
        ...
        {showPreview ? (
          <div className="w-full min-h-[200px] px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white whitespace-pre-wrap">
            {formData.body || <span className="text-white/30">No content</span>}
          </div>
        ) : (
          <textarea
            value={formData.body}
            ...
            rows={8}
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder:text-white/30 focus:outline-none focus:border-white/30 resize-none font-mono text-sm"
